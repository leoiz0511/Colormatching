...
public class CubeSpawner : MonoBehaviour
{
    ...
    private void Awake()
    {
        StartCoroutine("SpawnCubSet");
    }

    private IEnumerator SpawnCubSet()
    {
        while (true)
        {
            GameObject clone = Instantiate(cubesetPrefab, spawnPoint.position, Quaternion.identity);
            // 큐브셋 오브젝트의 자식으로 있는 9개의 큐브의 MeshRenderer 컴포넌트 정보
            MeshRenderer[] renderers = clone.GetComponentsInChildren<MeshRenderer>();
            // 9개의 큐브의 색상을 임의로 설정 (현재 스테이지에서 사용 가능한 큐브 색상으로 = CubeColors)
